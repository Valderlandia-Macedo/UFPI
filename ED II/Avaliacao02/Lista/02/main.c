...
void mainRead(Unity *unidades, int *contUnidades, const char *argv[])
{
	...
	char *promove;
	Text **promoveEnglish = malloc(sizeof(Text *));
	FILE *fPtr;
	fPtr = fopen(argv[1], "r");
		...
		else
		{
			...
			for(i = i + 1; linha[i - 1] != '\0'; i++)
			{
				...
				else
				{
					auxPortugues[j] = '\0';
					j = 0;
					treeInsert(&(unidades[(*contUnidades) - 1].tree), auxPortugues, auxIngles, promove, promoveEnglish, NULL);
				}
			}
		}
	}
